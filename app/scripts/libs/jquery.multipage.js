(function($) {
		...
		jQuery.fn.gotopage = function(page) {
			$(id + '_nav .multipage_next').html('Напред');
			...
			if (page==1) {
	            ...
	            if (settings.pages.length != 1 && settings.pages[page].title) {
	                $(id + '_nav .multipage_next').html('Напред: ' + settings.pages[page].title);
	            } else if (settings.pages.length == 1){
	                $(id + '_nav .multipage_next').addClass('submit');
	                $(id + '_nav .multipage_next').html('Изпрати');
	            } else {
	                $(id + '_nav .multipage_next').html('Напред');
	            }
				...
				if (settings.pages[page-2].title) { 
					$(id + '_nav .multipage_back').html('Назад: ' + settings.pages[page-2].title);
				} else {
					$(id + '_nav .multipage_back').html('Назад');				
				}
	...
	jQuery.fn.multipage = function(options) { 
		...
		settings = jQuery.extend({stayLinkable:false,submitLabel:'Изпрати',hideLegend:false,hideSubmit:true,generateNavigation:true,activeDot:'&nbsp;&#x25CF;',inactiveDot:'&nbsp;&middot;'},options);
		id = '#' + $(this).attr('id');
		var form = $(this);			
		...
		// $(id +' fieldset').hide();
			...
			if (settings.generateNavigation) { 
				if (settings.navigationFunction) { 
                    ...
                    $('<div class="multipage_nav" id="'+id_name+'_nav"><a href="#" class="multipage_back button">Назад</a><a href="#"  class="multipage_next button">Напред</a><span class="multipage_state"></span><div class="clearer"></div></div>').insertAfter(this);
				}
			}				
